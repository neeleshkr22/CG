  ...
  async execute(message, args) {
    ...
    if (lastUsed && now - lastUsed < COOLDOWN_TIME) {
      ...
      return message.reply(
        `🕒 Please wait **${secondsLeft} seconds** before using \`cmauction\` again.`
      );
    }
    ...
    if (!playerName || isNaN(bidAmount) || bidAmount <= 0) {
      return message.reply(
        '⚠️ Please use the format: `cmauction <playerName> <bidAmount>` (bid must be a number greater than 0).'
      );
    }
    ...
    if (!player) {
      return message.reply(
        `❌ Player **${playerName}** not found in the auction pool.`
      );
    }
    ...
    if (user.balance < bidAmount) {
      return message.reply(
        `💰 You don't have enough CG! Your current balance is **${user.balance} CG**.`
      );
    }
    ...
    user.teamOvr = user.players.reduce(
      (sum, p) => sum + (p.batting + p.bowling) / 2,
      0
    );
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${player.name}** was sold to **${message.author.username}**!`
      )
      .addFields(
        {
          name: '💸 Bid Amount',
          value: `${bidAmount.toLocaleString()} CG`,
          inline: true,
        },
        {
          name: '💰 Balance Left',
          value: `${user.balance.toLocaleString()} CG`,
          inline: true,
        },
        {
          name: '📈 Player Value',
          value: `${player.cardValue} CG`,
          inline: true,
        },
        ...
        {
          name: '📊 Team OVR',
          value: `${user.teamOvr.toFixed(1)}`,
          inline: true,
        },
        {
          name: '🧑‍🤝‍🧑 Players Owned',
          value: `${user.players.length}`,
          inline: true,
        }
      )
      .setFooter({
        text: 'Cricket Guru • Auction System',
